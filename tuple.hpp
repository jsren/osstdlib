...
namespace std
{
    ...
    template<size_t Index, typename... Ts>
    using TypeAtIndex = typename __detail::_TypeAtIndex<Index, Ts...>::type;

    template<typename... Ts>
    struct Tuple : __detail::_TupleBase<make_index_sequence<sizeof...(Ts)>, Ts...>
    {
    ...
    inline TypeAtIndex<Index, Ts...>& get(std::Tuple<Ts...>& tuple) {
        return static_cast<__detail::_TupleItem<TypeAtIndex<Index, Ts...>, Index>*>(&tuple)->item;
    }
    ...
    inline void for_each(Tuple<Ts...>& tuple, Func&& func) {
        __detail::_for_each_helper(tuple, static_cast<Func&&>(func), make_index_sequence<sizeof...(Ts)>());
    }
    ...
    constexpr __STD_DECLTYPE_AUTO apply(Func&& func, Tuple&& tuple) {
        return __detail::_apply(func, tuple, make_index_sequence<tuple_size<remove_reference_t<Tuple>>>{});
    }
#else
    template <class Func, class Tuple>
    constexpr __STD_DECLTYPE_AUTO apply(Func&& func, Tuple&& tuple) {
        return __detail::_apply(func, tuple, make_index_sequence<tuple_size<remove_reference_t<Tuple>>::value>{});
    }
#endif
